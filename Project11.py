...
#=============================================================    
    ...
    # st.write("page 3")
    st.spinner()
    with st.spinner(text='Please wait system is Processing'):
        time.sleep(2.5)
    # st.success('Done')
    ...
    # Progress
    # st.markdown("<h1 style='text-align:left; color: purple;font-size: 25px; '>Please Wait while system Is Processing :-</h1>", unsafe_allow_html=True)
    ...
    with open('Mysecret.key', 'rb') as my_public_key:
        key = my_public_key.read()
    f = Fernet(key)
    msg=st.text_input("")
    msg = msg.encode()
    ciphertext = f.encrypt(msg)
    
    clr = f.decrypt(ciphertext)
elif menu=="About App":
